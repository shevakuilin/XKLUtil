    ...
    switch (sender.tag) {
        case RECORE_BUTTON_TAG:{
            ...
            if (![audioRecorder isRecording]) {
                ...
                if ([recordLabel.text isEqualToString:@"点击开始"]) {
                    recordButton.tag = RECORE_BUTTON_TAG;
                    [self recordTimeStart];
                } else {
                    recordButton.tag = OVER_BUTTON_TAG;
                }
...
- (void)recordTimeTick {
    ...
    if (recordTime == 30 || recordButton.tag == OVER_BUTTON_TAG) {
        recordTime = 0;
        [audioRecorder stop];
